...
class Trainer:
    ...
    def train_epoch(self, epoch, dataset, should_print=False):
        ...
        for index, data in tqdm(enumerate(dataset)):
            ...
            if should_print:
                print(
                    "[Epoch %d] Train | Data %d (%d%%): loss: | path: %s"
                    % (epoch, index, percent, data.path),
                    " " * 20,
                    end="\r",
                )
            # try:
            ...
            if should_print:
                print(
                    "[Epoch %d] Train | Data %d (%d%%): loss: %.3f | path: %s"
                    % (epoch, index, percent, loss.item(), data.path),
                    " " * 20,
                    end="\n",
                )
            ...
            if self.weight_clipping is not None:
                torch.nn.utils.clip_grad_norm_(
                    self.model.parameters(), max_norm=self.weight_clipping
                )
            self.optimizer.step()
        ...
        for index, data in tqdm(enumerate(dataset)):
            ...
            if use_mask:
                ...
                _labels = torch.LongTensor(
                    [(-1 if i == 0 else i) for i in data.labels]
                ).to(self.device)
            else:
                _labels = data.labels
            ...
            if should_print:
                print(
                    ...
                    % (epoch, index, percent, acc, data.path),
                    " " * 30,
                    end="\n",
                )
...
def patch_chunks(dataset_folder):
    ...
    shutil.move(
        os.path.join(dataset_folder, "chunk"), os.path.join(dataset_folder, "chunk-old")
    )
    ...
    for chunk_path in dir_.iterdir():
        ...
        with open(
            str(chunk_path).replace("chunk-old", "chunk"), "w", encoding="utf-8"
        ) as ofile:
...
if __name__ == "__main__":
    ...
    train_dataset = TableDataset(train_path, with_cells=False, exts=["chunk", "rel"])
    ...
    model.to(device)
    criterion = torch.nn.CrossEntropyLoss(torch.FloatTensor([0.2, 1, 1]).to(device))
